...
// In-memory storage for devices, requests, and approved devices
let devices = [];
let requests = [];
let approvedDevices = [];
...
app.post('/api/request', (req, res) => {
  const { requesterDeviceId, targetDeviceId } = req.body;

  if (!requesterDeviceId || !targetDeviceId) {
    return res.status(400).json({ error: 'Requester and target device IDs are required' });
  }
  ...
  // Notify the target device about the new request
  io.emit('new-request', request);

  res.json({ message: 'Request sent successfully', requestId: request.requestId });
});

// 3. Approve request
app.post('/api/approve', (req, res) => {
  const { requestId, approverDeviceId } = req.body;
  ...
  if (request.targetDeviceId !== approverDeviceId) {
  ...
  approvedDevices.push({
    ...
    approvedDeviceId: request.targetDeviceId,
  });

  // Notify the requester about the approval
  io.emit('request-approved', request);

  res.json({ message: 'Request approved', requestId });
});

// 4. Get approved devices
app.get('/api/approved-devices/:deviceId', (req, res) => {
  const { deviceId } = req.params;

  const approved = approvedDevices.filter(
    (device) => device.approvedDeviceId === deviceId
  );

  res.json({ approvedDevices: approved });
});

// 5. Get live feed URL (mock implementation)
app.get('/api/live-feed/:deviceId', (req, res) => {
  const { deviceId } = req.params;
  ...
  res.json({
    deviceId,
    feedUrl: `rtsp://${device.ipAddress}/live`, // Mock RTSP URL
  });
});

// 6. Add overlay to live feed
app.post('/api/overlay', (req, res) => {
  const { deviceId, overlayText } = req.body;

  if (!deviceId || !overlayText) {
    return res.status(400).json({ error: 'Device ID and overlay text are required' });
  }

  // Notify the device to add the overlay
  io.emit('add-overlay', { deviceId, overlayText });

  res.json({ message: 'Overlay added successfully' });
});
